...
var bp_ajax_request = null;
 var activity_is_loading=false; //we use it to track if the the activity loading request is already going on  
...
jq(document).ready( function() {
   ...
   //BuddyPress Code starts here
    ...
    //for activity loading on mousehover
    jq('div.activity').on('mouseover','li.load-more a',function(event){
        var target = jq(event.target);
            load_more_activity(target);
    });
    
	/* Activity Stream event delegation */
	jq('div.activity').on('click', function(event) {
		...
		if ( target.hasClass('fav') || target.hasClass('unfav') ) {
                ...
                var parent = jq(target.parents('.activity-item').get(0));
                ...
                function(response) {
                    ...
                    if ( 'activity-favorites' == jq( '.item-list-tabs li.selected').attr('id') )
                        target.parents('.activity-item').slideUp(100);
                });
	...
	});//end of activity stream event delegation block
	...
	jq('div.activity').click( function(event) {
		...
		if ( target.hasClass('acomment-reply') || target.parent().hasClass('acomment-reply') ) {
			...
			jq('.ac-form').hide();
            jq('.activity-comments').removeClass('activity-now-commenting');
			...
			if ( form.parent().hasClass( 'activity-comments' ) )
				form.addClass('root');
            form.parent().addClass('activity-now-commenting');
	...
	jq(document).on('click','div.item-list-tabs', function(event) {
		if ( jq(this).hasClass('no-ajax') )
			return;
	...
	jq(document).on('change','li.filter select,div.filter select', function() {
    ...
    /**************************** Invite Friends on Group  ************************************/
	/***************************************************************************/
    //invite friends, add to list
	...
	jq(document).on('click',"#friend-list li a.remove", function() {
		jq('.ajax-loader').toggle();
  ...
  /**************************** Profile Visibility Settings  ************************************/
   /***************************************************************************/
	...
	jq(document).on('click','.friendship-button a', function() {
	...
	jq(document).on('click','.group-button a', function() {
...
/* Selecting unread and read messages in inbox */
	jq("#message-type-select").change(
		function() {
			var selection = jq(this).find(":selected").val();
          
			var checkboxes = jq(".thread-options input:checkbox");
			checkboxes.each( function(i) {
			...
			switch(selection) {
				case 'unread':
					var checkboxes = jq("li.unread  input:checkbox");
					break;
				case 'read':
					var checkboxes = jq("li.read input:checkbox");
					break;
			}
            ...
            if(jq.uniform!=undefined)
                        jq.uniform.update(checkboxes);
		}
	);
	...
	jq("a#delete_inbox_messages, a#delete_sentbox_messages").click( function() {
		checkboxes_tosend = '';
		checkboxes = jq("#message-threads .thread-options input[type='checkbox']");
...
jq(document).on('click','#message-threads a.message-delete',function(){
    
    var $this=jq(this);
    var url=$this.attr('href');//the url, we will need to find nonce from it
    var nonce=get_var_in_url(url, '_wpnonce');
    
    var attr_id=$this.attr('id');
    attr_id=attr_id.split('-');
    var thread_id=attr_id[2];
 ...
 function load_more_activity(target){
    
     if(activity_is_loading)
        return false;
    
     activity_is_loading=true;
...
function bp_init_objects(objects) {
	jq(objects).each( function(i) {
		...
		if ( null != jq.cookie('bp-' + objects[i] + '-scope') && jq('div.' + objects[i]).length ) {
			...
			jq('.item-list-tabs li#' + objects[i] + '-' + jq.cookie('bp-' + objects[i] + '-scope') + ', div.item-list-tabs#object-nav li.current,div.user-nav#object-nav li.current').addClass('selected');
		}
	});
}
