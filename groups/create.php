...
<div id="content">
	<div class="padder">
		...
		<form action="<?php bp_group_creation_form_action(); ?>" method="post" id="create-group-form" class="standard-form" enctype="multipart/form-data">
			<h3><?php _e( 'Create a Group', 'bp-magic' ); ?> &nbsp;<a class="button" href="<?php echo trailingslashit( bp_get_root_domain() . '/' . bp_get_groups_root_slug() ); ?>"><?php _e( 'Groups Directory', 'bp-magic' ); ?></a></h3>
			...
			<div class="clearfix item-body" id="group-create-body">
				...
				<?php if ( bp_is_group_creation_step( 'group-settings' ) ) : ?>
					...
					<?php if ( bp_is_active( 'forums' ) && bp_forums_is_installed_correctly() ) : ?>
						...
						<h4><?php _e( 'Group Forums', 'bp-magic' ); ?></h4>

						<p><?php _e( 'Should this group have a forum?', 'bp-magic' ); ?></p>

						<div class="checkbox">
							<label><input type="checkbox" name="group-show-forum" id="group-show-forum" value="1"<?php checked( bp_get_new_group_enable_forum(), true, true ); ?> /> <?php _e( 'Enable discussion forum', 'bp-magic' ); ?></label>
						</div>
				...
				<?php if ( bp_is_group_creation_step( 'group-avatar' ) ) : ?>
					...
					<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
						...
						<div class="main-column">
							...
							<p><?php _e( 'To skip the avatar upload process, hit the "Next Step" button.', 'bp-magic' ); ?></p>
						</div><!-- .main-column -->
					...
					<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
						...
						<h3><?php _e( 'Crop Group Avatar', 'bp-magic' ); ?></h3>
				...
				<?php if ( bp_is_group_creation_step( 'group-invites' ) ) : ?>
					...
					<?php if ( bp_is_active( 'friends' ) && bp_get_total_friend_count( bp_loggedin_user_id() ) ) : ?>
						...
						<div class="main-column">
							...
							<div id="message" class="info">
								<p><?php _e( 'Select people to invite from your friends list.', 'bp-magic' ); ?></p>
							</div>
								...
								<?php if ( bp_group_has_invites() ) : ?>
									...
									<?php while ( bp_group_invites() ) : bp_group_the_invite(); ?>
										...
										<li id="<?php bp_group_invite_item_id(); ?>">
											...
											<div class="action">
												<a class="remove" href="<?php bp_group_invite_user_remove_invite_url(); ?>" id="<?php bp_group_invite_item_id(); ?>"><?php _e( 'Remove Invite', 'bp-magic' ); ?></a>
											</div>
										</li>
					...
					<?php else : ?>
						...
						<div id="message" class="info">
							<p><?php _e( 'Once you have built up friend connections you will be able to invite others to your group. You can send invites any time in the future by selecting the "Send Invites" option when viewing your new group.', 'bp-magic' ); ?></p>
						</div>
				...
				<?php if ( 'crop-image' != bp_get_avatar_admin_step() ) : ?>
					...
					<div class="submit clearfix clear" id="previous-next">

						<?php /* Previous Button */ ?>
						<?php if ( ! bp_is_first_group_creation_step() ) : ?>

							<input type="button" value="<?php _e( 'Back to Previous Step', 'bp-magic' ); ?>" id="group-creation-previous" name="previous" onclick="location.href = '<?php bp_group_creation_previous_link(); ?>'" />
						...
						<?php if ( ! bp_is_last_group_creation_step() && ! bp_is_first_group_creation_step() ) : ?>
							...
							<input type="submit" value="<?php _e( 'Next Step', 'bp-magic' ); ?>" id="group-creation-next" name="save" />
						...
						<?php if ( bp_is_first_group_creation_step() ) : ?>
							...
							<input type="submit" value="<?php _e( 'Create Group and Continue', 'bp-magic' ); ?>" id="group-creation-create" name="save" />
						...
						<?php if ( bp_is_last_group_creation_step() ) : ?>
							...
							<input type="submit" value="<?php _e( 'Finish', 'bp-magic' ); ?>" id="group-creation-finish" name="save" />
